...
func (m *SSHModule) Run() error {
	...
	machines, apps, endpoints := m.store.GetMachinesByOpenTCPPort(22)
	for k, endpoint := range endpoints {
		if ipv4 := endpoint.Addr.To4(); ipv4 != nil {
			...
			if err != nil {
				m.logger.Errorf("fail to dial %s: %v", address, err)
				continue
			}
			service, err := p.Run(conn, time.Second, plugins.Target{})
			if err != nil {
				m.logger.Errorf("fail to run ssh discovery on %s: %v", address, err)
				continue
			}

			if service == nil || service.Raw == nil {
				m.logger.Errorf("ssh discovery returned no data for %s", address)
				continue
			}

			if err := json.Unmarshal(service.Raw, &s); err != nil {
				m.logger.Errorf("fail to unmarshal discovery results: %v", err)
				continue
			}
			...
			m.logger.WithField("host_key", s.HostKey).
			...
			if s.Banner != "" {
				...
				if machines[k].Platform == "" {
					...
					if banner.Platform != "" {
						entry = entry.WithField("platform", banner.Platform)
					}
				}
				if machines[k].Distribution == "" {
					machines[k].Distribution = banner.Distribution
					if banner.Distribution != "" {
						entry = entry.WithField("distribution", banner.Distribution)
					}
				}
				if machines[k].DistributionVersion == "" {
					machines[k].DistributionVersion = banner.DistributionVersion
					if banner.DistributionVersion != "" {
						entry = entry.WithField("distribution_version", banner.DistributionVersion)
					}
				}
				if apps[k].Name == "" {
					apps[k].Name = banner.Product
					if banner.Product != "" {
						entry = entry.WithField("name", banner.Product)
					}
				}
				if apps[k].Version == "" {
					apps[k].Version = banner.Version
					if banner.Version != "" {
						entry = entry.WithField("version", banner.Version)
					}
				}
				if apps[k].CPE == "" {
					apps[k].CPE = banner.CPE
					if banner.CPE != "" {
						entry = entry.WithField("cpe", banner.CPE)
					}
				}

				entry.Info("SSH banner analyzed")
			}
