...
func (m *MSIModule) Run() error {
	...
	if err != nil {
		m.logger.Errorf("error fetching system-wide apps: %v", err)
	}
	...
	if err != nil {
		m.logger.Errorf("error fetching 32-bit system-wide apps: %v", err)
	}
	userApps, err := getInstalledApps(registry.CURRENT_USER, `SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall`, logger)
	if err != nil {
		m.logger.Errorf("error fetching user-specific apps: %v", err)
	}
	...
	for _, pkg := range pkgs {
		...
		if out == nil {
			...
			m.logger.WithField("name", pkg.Name).
...
func getInstalledApps(root registry.Key, subKey string, logger logrus.FieldLogger) ([]*models.Package, error) {
	...
	for _, name := range names {
		...
		m.logger.Debugf("Looking for registry key: %v", subKeyPath)
		...
		go func(subKeyPath string) {
			...
			if value, _, err := subKey.GetStringValue("InstallLocation"); err == nil {
				m.logger.Debugf("InstallLocation: %v", value)
