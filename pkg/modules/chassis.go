...
func (m *ChassisModule) Run() error {
	...
	if !isSocketAvailable() {
		m.logger.Warn("dbus system bus is not available, skipping")
	...
	m.logger.Debug("opening system bus")
	...
	m.logger.Debug("getting chassis from org.freedesktop.hostname1")
	err = obj.Call("org.freedesktop.DBus.Properties.Get", 0, "org.freedesktop.hostname1", "Chassis").Store(&host.Chassis)
	if err != nil {
		return err
	}
	m.logger.WithField("chassis", host.Chassis).Info("chassis found through dbus")
