...
func (m *VMwareModule) Bind(config *puzzle.Config) error {
	if err := setDefault(config, m, "username", &m.Username, "VMware API username"); err != nil {
		return err
	}
	if err := setDefault(config, m, "password", &m.Password, "VMware API password"); err != nil {
		return err
	}
	return nil
}
		...
		for _, app := range machine.Applications() {
			for _, endpoint := range app.Endpoints {
				...
				if err != nil {
					...
					m.logger.WithField("ip", endpoint.Addr).
						WithField("port", endpoint.Port).
				...
				for _, vm := range vms {
					...
					updateMachine(machine, &moVM, m.logger.WithField("vm", vm.Name()), m.store)
				}
...
func updateMachine(parent *models.Machine, vm *mo.VirtualMachine, logger logrus.FieldLogger, s store.Store) {
	var err error
	if vm == nil {
		return
	}
...
netloop:
	for _, n := range networks {
		...
		for _, ipstr := range n.IpAddress {
			...
			machine = s.GetMachineByNetwork(ip, mac)
			if machine != nil {
				break netloop
			}
